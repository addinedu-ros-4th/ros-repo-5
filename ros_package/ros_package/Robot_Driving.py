...
class RobotDriver(Node):
        ...
        while rp.ok():
            if self.patrolling and not self.command_received:
                for i in range(len(goal_poses)):
                    ...
                    #     self.current_state = 'arrive at {}'.format(goal_pose)
    ...
    def robot_command_callback(self, request, response):
        ...
        if request.command == "human_detect":
            ...
            self.handle_human_detect(request.description)
    ...
    def handle_human_detect(self, description):
        self.get_logger().info('human_detect.')
        self.publish_robot_state()

        # description 값을 이용해 목표 위치 설정
        # goal_x, goal_y, goal_z = description[0,1,2]
        # goal_orientation_x, goal_orientation_y, goal_orientation_z, goal_orientation_w = description[3,4,5,6]
